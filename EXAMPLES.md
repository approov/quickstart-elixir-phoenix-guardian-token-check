...
[Approov](https://approov.io) is an API security solution used to verify that requests received by your backend services originate from trusted versions of your mobile apps, and here you can find the Hello servers examples that are the base for the Approov [quickstarts](/docs) for the Elixir Phoenix framework.
...
If you are looking for the Approov quickstarts to integrate Approov in your Phoenix API server then you can find them [here](/docs).
...
Do not forget to properly setup the `.env` file in the root of this repo before you run any of the server examples with the docker stack.

```bash
cp .env.example .env
```

Edit the file and add the [dummy secret](/README.md#the-dummy-secret) to it in order to be able to test the Approov integration with the provided [Postman collection](https://github.com/approov/postman-collections/blob/master/quickstarts/hello-world/hello-world.postman_curl_requests_examples.md).
...
Now, you are ready to start using the Docker stack for the Elixir Phoenix framework.

### Fetch and Compile Dependencies

Instead of fetching and compiling the dependencies when building the docker image, we will map the local volume to the running docker container to allow for the same workflow as when not using docker while developing or playing around with the server examples.

To fetch and compile all the dependencies for each server:

```bash
sudo docker-compose run --rm unprotected-server bash -c "mix deps.get && mix compile"
sudo docker-compose run --rm approov-token-check bash -c "mix deps.get && mix compile"
sudo docker-compose run --rm approov-token-binding-check bash -c "mix deps.get && mix compile"
```

To get the same workflow as you would with a local Elixir and Phoenix installation you just need to get a shell inside the docker container. See the second option for each server example in the below command examples, that looks like `sudo docker-compose run --rm --service-ports <docker-compose-service-here> zsh`.
